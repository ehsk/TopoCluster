...
def Epanech(ptbl, pid, dist, cur, pointgrid="None"):
    ...
    if pointgrid=="None":
        cur.execute("SELECT p2.gid, .75 * (1 - power((ST_Distance(p1.geog, p2.geog)/%s), 2)) FROM %s as p1, %s as p2 WHERE p1.uid = %s and ST_DWithin(p1.geog, p2.geog, %s); " % (dist, ptbl, ptbl, '%s', dist), (pid, ) )
    else:
        cur.execute("SELECT p1.gid, .75 * (1 - power((ST_Distance(p1.geog, p2.geog)/%s), 2)) FROM %s as p1, %s as p2 WHERE p2.gid = %s and (ST_Distance(p1.geog, p2.geog) < %s);" % (dist, ptbl, pointgrid, '%s', dist), (pid, ))
    return cur.fetchall()
...
def Quartic_biweight(ptbl, pid, dist, cur, pointgrid="None"):
    ...
    if pointgrid=="None":
        cur.execute("SELECT p2.gid, power(1 - power((ST_Distance(p1.geog, p2.geog)/%s), 2), 2) FROM %s as p1, %s as p2 WHERE p1.uid = %s and ST_DWithin(p1.geog, p2.geog, %s); " % (dist, ptbl, ptbl, '%s', dist), (pid, ) )
    else:
...
def Uniform(ptbl, pid, dist, cur, pointgrid="None"):
    ...
    if pointgrid=="None":
        cur.execute("SELECT p2.gid, 1.0 FROM %s as p1, %s as p2 WHERE p1.uid = %s and ST_DWithin(p1.geog, p2.geog, %s); " % (ptbl, ptbl, '%s', dist), (pid, ) )
    elif pointgrid=="Only":
